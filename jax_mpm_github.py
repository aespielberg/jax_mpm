...
def allocate_arrays():
  ...
  global grid_m_in, grid_v_in, grid_v_out, loss, x_avg, index_array
  ...
  loss = jnp.zeros(())
  x_avg = jnp.zeros((dim,))
  index_array = jnp.zeros((n_grid, n_grid, dim))
  
  for i in range(n_grid):
    for j in range(n_grid):
      index_array.at[i, j].set(jnp.array([i, j]))
...
p1 = jit(vmap(p2g_contrib, in_axes=(0, 0, 0, 0, 0, None)))
p2 = jit(p2g_accum)
go = jit(vmap(vmap(grid_op)))
g = jit(vmap(g2p, in_axes=(0, None)))
ca = jit(compute_actuation)
  

def forward(x, v, C, F, actuator_id, actuation):
  for t in range(2000):
    actuation = ca(t)
...
def main():
  ...
  #for i in range(100):
  #  print(forward(x, v, C, F, actuator_id, actuation))
  
  #Question 1: Why is this running out of memory?
  forward_grad = grad(forward)
  gradient = forward_grad(x, v, C, F, actuator_id, actuation)
  
  #Question 2: Why is this so slow to compile?
  f = jit(forward)
  
  #print(f(x, v, C, F, actuator_id, actuation))
  #print(timeit.timeit(lambda : f(x, v, C, F, actuator_id, actuation).block_until_ready(), number=number) / number)
